...
if not GPU_AVAILABLE:
    warnings.warn("No GPU backend available. Install CuPy or PyTorch with CUDA support for GPU acceleration.")

@dataclass
class GPUConfig:
    """Configuration for GPU operations"""
    enabled: bool = GPU_AVAILABLE
    backend: Optional[str] = GPU_BACKEND
    device_id: int = 0
...
class GPUTensorOps:
    ...
    # Creation operations
    def zeros(self, shape: Union[int, Tuple], dtype=None) -> GPUTensor:
    ...
    def ones(self, shape: Union[int, Tuple], dtype=None) -> GPUTensor:
    ...
    def eye(self, n: int, m: Optional[int] = None, dtype=None) -> GPUTensor:
    ...
    def random(self, shape: Union[int, Tuple], dtype=None) -> GPUTensor:
    ...
    def add(self, a: GPUTensor, b: Union[GPUTensor, float]) -> GPUTensor:
        """Add tensors on GPU"""
    ...
    def multiply(self, a: GPUTensor, b: Union[GPUTensor, float]) -> GPUTensor:
    ...
    def sum(self, tensor: GPUTensor, axis: Optional[int] = None) -> Union[GPUTensor, float]:
    ...
    def mean(self, tensor: GPUTensor, axis: Optional[int] = None) -> Union[GPUTensor, float]:
    ...
    def std(self, tensor: GPUTensor, axis: Optional[int] = None) -> Union[GPUTensor, float]:
...
def get_gpu_ops(config: Optional[GPUConfig] = None) -> GPUTensorOps:
...
def benchmark_gpu_vs_cpu():
    ...
    for size in sizes:
        ...
        if GPU_AVAILABLE:
            ...
            start = time.perf_counter()
            for _ in range(10):
                c_gpu = gpu_ops.matmul(a_gpu, b_gpu)
