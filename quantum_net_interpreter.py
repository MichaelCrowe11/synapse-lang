...
class BB84Protocol:
    ...
    def prepare_qubits(self, bits: list[int], bases: list[str]) -> list[np.ndarray]:
        ...
        qubits = []
        for bit, basis in zip(bits, bases, strict=False):
            if basis == "Z":
                if bit == 0:
    ...
    def measure_qubits(self, qubits: list[np.ndarray], bases: list[str]) -> list[int]:
        ...
        results = []
        for qubit, basis in zip(qubits, bases, strict=False):
            if basis == "Z":
    ...
    def sift_key(self, bits: list[int], alice_bases: list[str],
        ...
        sifted_key = []
        for _i, (bit, a_basis, b_basis) in enumerate(zip(bits, alice_bases, bob_bases, strict=False)):
            if a_basis == b_basis:
    ...
    def estimate_error_rate(self, alice_key: list[int], bob_key: list[int],
        ...
        errors = sum(a != b for a, b in zip(alice_key[:sample_size], bob_key[:sample_size], strict=False))
...
class E91Protocol:
    ...
    def measure_correlation(self, measurements_a: list[int], measurements_b: list[int]) -> float:
        ...
        correlation = sum((-1)**(a + b) for a, b in zip(measurements_a, measurements_b, strict=False))
...
class QuantumNetInterpreter(ASTVisitor):
    ...
    def _get_link_between(self, node1: str, node2: str) -> NetworkLink | None:
