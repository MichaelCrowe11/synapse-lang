...
class SynapseTensor:
    ...
    def transpose(self, axes: list[int] | None = None) -> "SynapseTensor":
        """Transpose tensor"""
    ...
    def sum(self, axis: int | tuple[int, ...] | None = None) -> "SynapseTensor":
    ...
    def mean(self, axis: int | tuple[int, ...] | None = None) -> "SynapseTensor":
    ...
    def std(self, axis: int | tuple[int, ...] | None = None) -> "SynapseTensor":
        """Standard deviation"""
        result_data = np.std(self.data, axis=axis)
        return SynapseTensor(result_data)

    def normalize(self, axis: int | None = None) -> "SynapseTensor":
        """Normalize tensor"""
...
class TensorOperations:
    ...
    @staticmethod
    def zeros(shape: int | tuple[int, ...], dtype=np.float64) -> SynapseTensor:
        """Create zero tensor"""
        return SynapseTensor(np.zeros(shape, dtype=dtype))

    @staticmethod
    def ones(shape: int | tuple[int, ...], dtype=np.float64) -> SynapseTensor:
        """Create ones tensor"""
        return SynapseTensor(np.ones(shape, dtype=dtype))

    @staticmethod
    def random(shape: int | tuple[int, ...],
    ...
    @staticmethod
    def eye(n: int, m: int | None = None, dtype=np.float64) -> SynapseTensor:
    ...
    @staticmethod
    def gradient(tensor: SynapseTensor, axis: int | None = None) -> SynapseTensor:
        """Compute gradient"""
        if axis is None:
    ...
    def apply_function(tensor: SynapseTensor, func: callable,
                      deriv_func: callable | None = None) -> SynapseTensor:
